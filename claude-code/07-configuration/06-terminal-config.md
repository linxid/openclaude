...
为了进一步自定义 Claude Code 界面本身，您可以配置一个[自定义状态行](/claude-code/07-configuration/09-statusline)来显示上下文信息，如当前模型、工作目录或 git 分支在您的终端底部。
...
对于高级通知处理，您可以创建[通知钩子](/claude-code/04-build-with-claude/03-hooks#notification)来运行您自己的逻辑。
...
请参阅[交互模式](/claude-code/08-reference/02-interactive-mode#vim-editor-mode)获取完整参考。
