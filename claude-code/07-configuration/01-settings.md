...
* **Managed 设置**：对于需要集中控制的组织，Claude Code 支持可以部署到系统目录的 `managed-settings.json` 和 `managed-mcp.json` 文件：
  ...
  * Windows：`C:\Program Files\ClaudeCode\`
  ...
  有关详细信息，请参阅 [Managed 设置](/claude-code/07-configuration/02-permissions#managed-settings) 和 [Managed MCP 配置](/claude-code/04-build-with-claude/05-mcp#managed-mcp-configuration)。

  <Note>
    Managed 部署还可以使用 `strictKnownMarketplaces` 限制**插件市场添加**。有关更多信息，请参阅 [Managed 市场限制](/claude-code/04-build-with-claude/09-plugin-marketplaces#managed-marketplace-restrictions)。
  </Note>
* **其他配置**存储在 `~/.claude.json` 中。此文件包含您的偏好（主题、通知设置、编辑器模式）、OAuth 会话、用户和本地作用域的 [MCP server](/claude-code/04-build-with-claude/05-mcp) 配置、每个项目的状态（允许的工具、信任设置）和各种缓存。项目作用域的 MCP servers 单独存储在 `.mcp.json` 中。
...
| `hooks`                           | 配置自定义命令以在生命周期事件处运行。有关格式，请参阅 [hooks 文档](/claude-code/04-build-with-claude/03-hooks)                                                                                                              | 请参阅 [hooks](/claude-code/04-build-with-claude/03-hooks)                                        |
| `disableAllHooks`                 | 禁用所有 [hooks](/claude-code/04-build-with-claude/03-hooks)                                                                                                                                        | `true`                                                           |
| `allowManagedHooksOnly`           | （仅 Managed 设置）防止加载用户、项目和插件 hooks。仅允许 managed hooks 和 SDK hooks。请参阅 [Hook 配置](#hook-configuration)                                                                 | `true`                                                           |
| `allowManagedPermissionRulesOnly` | （仅 Managed 设置）防止用户和项目设置定义 `allow`、`ask` 或 `deny` 权限规则。仅应用 managed 设置中的规则。请参阅 [仅 Managed 设置](/claude-code/07-configuration/02-permissions#managed-only-settings)                             | `true`                                                           |
| `model`                           | 覆盖 Claude Code 使用的默认模型                                                                                                                                            | `"claude-sonnet-4-5-20250929"`                                   |
| `otelHeadersHelper`               | 生成动态 OpenTelemetry 标头的脚本。在启动时和定期运行（请参阅 [动态标头](/claude-code/06-administration/06-monitoring-usage#dynamic-headers)）                                                                           | `/bin/generate_otel_headers.sh`                                  |
| `statusLine`                      | 配置自定义状态行以显示上下文。请参阅 [`statusLine` 文档](/claude-code/07-configuration/09-statusline)                                                                                                           | `{"type": "command", "command": "~/.claude/statusline.sh"}`      |
...
| `outputStyle`                     | 配置输出样式以调整系统提示。请参阅 [输出样式文档](/claude-code/07-configuration/08-output-styles)                                                                                                                  | `"Explanatory"`                                                  |
...
| `allowedMcpServers`               | 在 managed-settings.json 中设置时，用户可以配置的 MCP servers 的允许列表。未定义 = 无限制，空数组 = 锁定。适用于所有作用域。拒绝列表优先。请参阅 [Managed MCP 配置](/claude-code/04-build-with-claude/05-mcp#managed-mcp-configuration)              | `[{ "serverName": "github" }]`                                   |
| `deniedMcpServers`                | 在 managed-settings.json 中设置时，明确阻止的 MCP servers 的拒绝列表。适用于所有作用域，包括 managed servers。拒绝列表优先于允许列表。请参阅 [Managed MCP 配置](/claude-code/04-build-with-claude/05-mcp#managed-mcp-configuration)           | `[{ "serverName": "filesystem" }]`                               |
| `strictKnownMarketplaces`         | 在 managed-settings.json 中设置时，用户可以添加的插件市场的允许列表。未定义 = 无限制，空数组 = 锁定。仅适用于市场添加。请参阅 [Managed 市场限制](/claude-code/04-build-with-claude/09-plugin-marketplaces#managed-marketplace-restrictions)         | `[{ "source": "github", "repo": "acme-corp/plugins" }]`          |
| `awsAuthRefresh`                  | 修改 `.aws` 目录的自定义脚本（请参阅 [高级凭证配置](/claude-code/06-administration/02-amazon-bedrock#advanced-credential-configuration)）                                                                         | `aws sso login --profile myprofile`                              |
| `awsCredentialExport`             | 输出包含 AWS 凭证的 JSON 的自定义脚本（请参阅 [高级凭证配置](/claude-code/06-administration/02-amazon-bedrock#advanced-credential-configuration)）                                                                   | `/bin/generate_aws_grant.sh`                                     |
| `alwaysThinkingEnabled`           | 为所有会话默认启用 [扩展思考](/claude-code/02-core-concepts/03-common-workflows#use-extended-thinking-thinking-mode)。通常通过 `/config` 命令而不是直接编辑来配置                                                         | `true`                                                           |
...
| `teammateMode`                    | [agent team](/claude-code/05-deployment/04-agent-teams) 队友的显示方式：`auto`（在 tmux 或 iTerm2 中选择分割窗格，否则为进程内）、`in-process` 或 `tmux`。请参阅 [设置 agent teams](/claude-code/05-deployment/04-agent-teams#set-up-agent-teams) | `"in-process"`                                                   |
...
| `deny`                         | 拒绝工具使用的权限规则数组。使用此选项从 Claude Code 访问中排除敏感文件。请参阅 [权限规则语法](#permission-rule-syntax) 和 [Bash 权限限制](/claude-code/07-configuration/02-permissions#tool-specific-permission-rules) | `[ "WebFetch", "Bash(curl *)", "Read(./.env)", "Read(./secrets/**)" ]` |
| `additionalDirectories`        | Claude 有权访问的其他[工作目录](/claude-code/07-configuration/02-permissions#working-directories)                                                                                      | `[ "../docs/" ]`                                                       |
| `defaultMode`                  | 打开 Claude Code 时的默认[权限模式](/claude-code/07-configuration/02-permissions#permission-modes)                                                                                    | `"acceptEdits"`                                                        |
| `disableBypassPermissionsMode` | 设置为 `"disable"` 以防止激活 `bypassPermissions` 模式。这禁用 `--dangerously-skip-permissions` 命令行标志。请参阅 [managed 设置](/claude-code/07-configuration/02-permissions#managed-settings)     | `"disable"`                                                            |
...
有关完整的规则语法参考，包括通配符行为、Read、Edit、WebFetch、MCP 和 Task 规则的工具特定模式，以及 Bash 模式的安全限制，请参阅 [权限规则语法](/claude-code/07-configuration/02-permissions#permission-rule-syntax)。

### Sandbox 设置

配置高级沙箱行为。沙箱将 bash 命令与您的文件系统和网络隔离。有关详细信息，请参阅 [Sandboxing](/claude-code/07-configuration/10-sandboxing)。
...
该命令使用与 [hooks](/claude-code/04-build-with-claude/03-hooks) 相同的环境变量运行，包括 `CLAUDE_PROJECT_DIR`。它通过 stdin 接收包含 `query` 字段的 JSON：
...
Subagent 文件定义具有自定义提示和工具权限的专门 AI 助手。在 [subagents 文档](/claude-code/04-build-with-claude/02-sub-agents) 中了解有关创建和使用 subagents 的更多信息。
...
**仅 Managed 设置**：控制用户可以添加哪些插件市场。此设置只能在 [`managed-settings.json`](/claude-code/07-configuration/02-permissions#managed-settings) 中配置，为管理员提供对市场源的严格控制。
...
<Note>
  基于 URL 的市场仅下载 `marketplace.json` 文件。它们不从服务器下载插件文件。基于 URL 的市场中的插件必须使用外部源（GitHub、npm 或 git URL），而不是相对路径。对于具有相对路径的插件，请改用基于 Git 的市场。有关详细信息，请参阅 [故障排除](/claude-code/04-build-with-claude/09-plugin-marketplaces#plugins-with-relative-paths-fail-in-url-based-marketplaces)。
</Note>
...
有关面向用户的文档，请参阅 [Managed 市场限制](/claude-code/04-build-with-claude/09-plugin-marketplaces#managed-marketplace-restrictions)。
...
在 [插件文档](/claude-code/04-build-with-claude/06-plugins) 中了解有关插件系统的更多信息。
...
| `ANTHROPIC_DEFAULT_HAIKU_MODEL`                | 请参阅 [模型配置](/claude-code/07-configuration/04-model-config#environment-variables)                                                                                                                                                                                                                                       |     |
| `ANTHROPIC_DEFAULT_OPUS_MODEL`                 | 请参阅 [模型配置](/claude-code/07-configuration/04-model-config#environment-variables)                                                                                                                                                                                                                                       |     |
| `ANTHROPIC_DEFAULT_SONNET_MODEL`               | 请参阅 [模型配置](/claude-code/07-configuration/04-model-config#environment-variables)                                                                                                                                                                                                                                       |     |
| `ANTHROPIC_FOUNDRY_API_KEY`                    | Microsoft Foundry 身份验证的 API 密钥（请参阅 [Microsoft Foundry](/claude-code/06-administration/04-microsoft-foundry)）                                                                                                                                                                                                           |     |
| `ANTHROPIC_FOUNDRY_BASE_URL`                   | Foundry 资源的完整基础 URL（例如 `https://my-resource.services.ai.azure.com/anthropic`）。`ANTHROPIC_FOUNDRY_RESOURCE` 的替代方案（请参阅 [Microsoft Foundry](/claude-code/06-administration/04-microsoft-foundry)）                                                                                                                         |     |
| `ANTHROPIC_FOUNDRY_RESOURCE`                   | Foundry 资源名称（例如 `my-resource`）。如果未设置 `ANTHROPIC_FOUNDRY_BASE_URL`，则为必需（请参阅 [Microsoft Foundry](/claude-code/06-administration/04-microsoft-foundry)）                                                                                                                                                                   |     |
| `ANTHROPIC_MODEL`                              | 要使用的模型设置的名称（请参阅 [模型配置](/claude-code/07-configuration/04-model-config#environment-variables)）                                                                                                                                                                                                                          |     |
| `ANTHROPIC_SMALL_FAST_MODEL`                   | **已弃用** 用于后台任务的 [Haiku 级模型](/claude-code/06-administration/08-costs) 的名称                                                                                                                                                                                                                                               |     |
...
| `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE`              | 设置触发自动压缩的上下文容量百分比（1-100）。默认情况下，自动压缩在约 95% 容量时触发。使用较低的值（如 `50`）以更早压缩。高于默认阈值的值无效。适用于主对话和 subagents。此百分比与 [状态行](/claude-code/07-configuration/09-statusline) 中可用的 `context_window.used_percentage` 字段对齐                                                                                                                  |     |
...
| `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`         | 设置为 `1` 以启用 [agent teams](/claude-code/05-deployment/04-agent-teams)。Agent teams 是实验性的，默认禁用                                                                                                                                                                                                                        |     |
...
| `CLAUDE_CODE_EFFORT_LEVEL`                     | 为支持的模型设置努力级别。值：`low`、`medium`、`high`（默认）。较低的努力更快且更便宜，较高的努力提供更深入的推理。目前仅支持 Opus 4.6。请参阅 [调整努力级别](/claude-code/07-configuration/04-model-config#adjust-effort-level)                                                                                                                                                     |     |
...
| `CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY`          | 设置为 `1` 以禁用"Claude 做得怎么样？"会话质量调查。当使用第三方提供商或禁用遥测时也会禁用。请参阅 [会话质量调查](/claude-code/09-resources/02-data-usage#session-quality-surveys)                                                                                                                                                                                |     |
...
| `CLAUDE_CODE_TASK_LIST_ID`                     | 跨会话共享任务列表。在多个 Claude Code 实例中设置相同的 ID 以协调共享任务列表。请参阅 [任务列表](/claude-code/08-reference/02-interactive-mode#task-list)                                                                                                                                                                                               |     |
| `CLAUDE_CODE_TEAM_NAME`                        | 此队友所属的 agent team 的名称。在 [agent team](/claude-code/05-deployment/04-agent-teams) 成员上自动设置                                                                                                                                                                                                                            |     |
...
| `CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION`         | 设置为 `false` 以禁用提示建议（`/config` 中的"提示建议"切换）。这些是 Claude 响应后出现在您的提示输入中的灰显预测。请参阅 [提示建议](/claude-code/08-reference/02-interactive-mode#prompt-suggestions)                                                                                                                                                              |     |
| `CLAUDE_CODE_ENABLE_TASKS`                     | 设置为 `false` 以临时恢复到之前的 TODO 列表而不是任务跟踪系统。默认：`true`。请参阅 [任务列表](/claude-code/08-reference/02-interactive-mode#task-list)                                                                                                                                                                                              |     |
| `CLAUDE_CODE_ENABLE_TELEMETRY`                 | 设置为 `1` 以启用用于指标和日志记录的 OpenTelemetry 数据收集。在配置 OTel 导出器之前需要。请参阅 [监控](/claude-code/06-administration/06-monitoring-usage)                                                                                                                                                                                                 |     |
...
| `CLAUDE_CODE_MAX_OUTPUT_TOKENS`                | 为大多数请求设置最大输出令牌数。默认：32,000。最大：64,000。增加此值会减少在 [自动压缩](/claude-code/06-administration/08-costs#reduce-token-usage) 触发之前可用的有效上下文窗口。                                                                                                                                                                                        |     |
| `CLAUDE_CODE_OTEL_HEADERS_HELPER_DEBOUNCE_MS`  | 刷新动态 OpenTelemetry 标头的间隔（以毫秒为单位）（默认：1740000 / 29 分钟）。请参阅 [动态标头](/claude-code/06-administration/06-monitoring-usage#dynamic-headers)                                                                                                                                                                                    |     |
| `CLAUDE_CODE_PLAN_MODE_REQUIRED`               | 在需要计划批准的 [agent team](/claude-code/05-deployment/04-agent-teams) 队友上自动设置为 `true`。只读：由 Claude Code 在生成队友时设置。请参阅 [需要计划批准](/claude-code/05-deployment/04-agent-teams#require-plan-approval-for-teammates)                                                                                                                                    |     |
...
| `CLAUDE_CODE_SUBAGENT_MODEL`                   | 请参阅 [模型配置](/claude-code/07-configuration/04-model-config)                                                                                                                                                                                                                                                             |     |
| `CLAUDE_CODE_USE_BEDROCK`                      | 使用 [Bedrock](/claude-code/06-administration/02-amazon-bedrock)                                                                                                                                                                                                                                                         |     |
| `CLAUDE_CODE_USE_FOUNDRY`                      | 使用 [Microsoft Foundry](/claude-code/06-administration/04-microsoft-foundry)                                                                                                                                                                                                                                            |     |
| `CLAUDE_CODE_USE_VERTEX`                       | 使用 [Vertex](/claude-code/06-administration/03-google-vertex-ai)                                                                                                                                                                                                                                                        |     |
...
| `ENABLE_TOOL_SEARCH`                           | 控制 [MCP 工具搜索](/claude-code/04-build-with-claude/05-mcp#scale-with-mcp-tool-search)。值：`auto`（默认，在 10% 上下文处启用）、`auto:N`（自定义阈值，例如 `auto:5` 表示 5%）、`true`（始终开启）、`false`（禁用）                                                                                                                                                   |     |
...
| `MAX_THINKING_TOKENS`                          | 覆盖 [扩展思考](https://platform.claude.com/docs/en/build-with-claude/extended-thinking) 令牌预算。默认情况下，思考在最大预算（31,999 个令牌）处启用。使用此选项限制预算（例如 `MAX_THINKING_TOKENS=10000`）或完全禁用思考（`MAX_THINKING_TOKENS=0`）。对于 Opus 4.6，思考深度由 [努力级别](/claude-code/07-configuration/04-model-config#adjust-effort-level) 控制，此变量被忽略，除非设置为 `0` 以禁用思考。 |     |
| `MCP_CLIENT_SECRET`                            | 需要 [预配置凭证](/claude-code/04-build-with-claude/05-mcp#use-pre-configured-oauth-credentials) 的 MCP servers 的 OAuth 客户端密钥。在使用 `--client-secret` 添加服务器时避免交互式提示                                                                                                                                                                 |     |
| `MCP_OAUTH_CALLBACK_PORT`                      | OAuth 重定向回调的固定端口，作为使用 [预配置凭证](/claude-code/04-build-with-claude/05-mcp#use-pre-configured-oauth-credentials) 添加 MCP 服务器时 `--callback-port` 的替代方案                                                                                                                                                                          |     |
| `MCP_TIMEOUT`                                  | MCP 服务器启动的超时（以毫秒为单位）                                                                                                                                                                                                                                                                        |     |
| `MCP_TOOL_TIMEOUT`                             | MCP 工具执行的超时（以毫秒为单位）                                                                                                                                                                                                                                                                         |     |
| `NO_PROXY`                                     | 将直接发出请求的域和 IP 列表，绕过代理                                                                                                                                                                                                                                                                       |     |
| `SLASH_COMMAND_TOOL_CHAR_BUDGET`               | 显示给 [Skill 工具](/claude-code/04-build-with-claude/01-skills#control-who-invokes-a-skill) 的技能元数据的最大字符数（默认：15000）。为了向后兼容而保留的旧名称。                                                                                                                                                                                             |     |
...
| **MCPSearch**       | 当启用 [工具搜索](/claude-code/04-build-with-claude/05-mcp#scale-with-mcp-tool-search) 时搜索和加载 MCP 工具                                                                                 | 否    |
| **NotebookEdit**    | 修改 Jupyter notebook 单元格                                                                                                                         | 是    |
| **Read**            | 读取文件的内容                                                                                                                                         | 否    |
| **Skill**           | 在主对话中执行 [skill](/claude-code/04-build-with-claude/01-skills#control-who-invokes-a-skill)                                                                                      | 是    |
...
| **Write**           | 创建或覆盖文件                                                                                                                                         | 是    |
| **LSP**             | 通过语言服务器的代码智能。在文件编辑后自动报告类型错误和警告。还支持导航操作：跳转到定义、查找引用、获取类型信息、列出符号、查找实现、跟踪调用层次结构。需要 [代码智能插件](/claude-code/04-build-with-claude/08-discover-plugins#code-intelligence) 及其语言服务器二进制文件 | 否    |

权限规则可以使用 `/allowed-tools` 或在 [权限设置](/claude-code/07-configuration/01-settings#available-settings) 中配置。另请参阅 [工具特定权限规则](/claude-code/07-configuration/02-permissions#tool-specific-permission-rules)。
...
有关选项 3 的更多详细信息，请参阅 [SessionStart hooks](/claude-code/04-build-with-claude/03-hooks#persist-environment-variables)。

### 使用 hooks 扩展工具

您可以使用 [Claude Code hooks](/claude-code/04-build-with-claude/04-hooks-guide) 在任何工具执行之前或之后运行自定义命令。

例如，您可以在 Claude 修改 Python 文件后自动运行 Python 格式化程序，或通过阻止对某些路径的 Write 操作来防止修改生产配置文件。

## 另请参阅

* [权限](/claude-code/07-configuration/02-permissions)：权限系统、规则语法、工具特定模式和 managed 策略
* [身份验证](/claude-code/07-configuration/02-permissions)：设置用户对 Claude Code 的访问
* [故障排除](/claude-code/09-resources/04-troubleshooting)：常见配置问题的解决方案
