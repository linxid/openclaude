...
| `claude mcp`                    | 配置 Model Context Protocol (MCP) 服务器 | 请参阅 [Claude Code MCP 文档](/claude-code/04-build-with-claude/05-mcp)。        |
...
| `--agents`                             | 通过 JSON 动态定义自定义 [subagents](/claude-code/04-build-with-claude/02-sub-agents)（参见下面的格式）                                                                      | `claude --agents '{"reviewer":{"description":"Reviews code","prompt":"You are a code reviewer"}}'` |
...
| `--allowedTools`                       | 无需提示权限即可执行的工具。请参阅 [权限规则语法](/claude-code/07-configuration/01-settings#permission-rule-syntax) 了解模式匹配。要限制可用的工具，请改用 `--tools`                             | `"Bash(git log *)" "Bash(git diff *)" "Read"`                                                      |
...
| `--chrome`                             | 启用 [Chrome 浏览器集成](/claude-code/03-platforms/03-chrome) 用于网络自动化和测试                                                                                  | `claude --chrome`                                                                                  |
...
| `--no-chrome`                          | 为此会话禁用 [Chrome 浏览器集成](/claude-code/03-platforms/03-chrome)                                                                                         | `claude --no-chrome`                                                                               |
...
| `--permission-mode`                    | 以指定的 [权限模式](/claude-code/07-configuration/02-permissions#permission-modes) 开始                                                                          | `claude --permission-mode plan`                                                                    |
...
| `--remote`                             | 在 claude.ai 上创建新的 [网络会话](/claude-code/03-platforms/01-claude-code-on-the-web)，提供任务描述                                                               | `claude --remote "Fix the login bug"`                                                              |
...
| `--teleport`                           | 在本地终端中恢复 [网络会话](/claude-code/03-platforms/01-claude-code-on-the-web)                                                                               | `claude --teleport`                                                                                |
| `--teammate-mode`                      | 设置 [代理团队](/claude-code/05-deployment/04-agent-teams) 队友的显示方式：`auto`（默认）、`in-process` 或 `tmux`。请参阅 [设置代理团队](/claude-code/05-deployment/04-agent-teams#set-up-agent-teams)   | `claude --teammate-mode in-process`                                                                |
...
| 字段            | 必需 | 描述                                                                        |
| :------------ | :- | :------------------------------------------------------------------------ |
| `description` | 是  | 自然语言描述何时应调用 subagent                                                      |
| `prompt`      | 是  | 指导 subagent 行为的系统提示                                                       |
| `tools`       | 否  | subagent 可以使用的特定工具数组（例如，`["Read", "Edit", "Bash"]`）。如果省略，继承所有工具           |
...
有关创建和使用 subagents 的更多详情，请参阅 [subagents 文档](/claude-code/04-build-with-claude/02-sub-agents)。

### 系统提示标志

Claude Code 提供四个标志用于自定义系统提示，每个标志都有不同的用途：

| 标志                            | 行为              | 模式      | 用例                          |
| :---------------------------- | :-------------- | :------ | :-------------------------- |
| `--system-prompt`             | **替换**整个默认提示    | 交互 + 打印 | 完全控制 Claude 的行为和指令          |
| `--system-prompt-file`        | **替换**为文件内容     | 仅打印     | 从文件加载提示以实现可重现性和版本控制         |
| `--append-system-prompt`      | **附加**到默认提示     | 交互 + 打印 | 添加特定指令同时保持默认 Claude Code 行为 |
| `--append-system-prompt-file` | **附加**文件内容到默认提示 | 仅打印     | 从文件加载额外指令同时保持默认值            |
...
对于大多数用例，建议使用 `--append-system-prompt` 或 `--append-system-prompt-file`，因为它们保留 Claude Code 的内置功能同时添加您的自定义要求。仅当您需要完全控制系统提示时才使用 `--system-prompt` 或 `--system-prompt-file`。

## 另请参阅

* [Chrome 扩展](/claude-code/03-platforms/03-chrome) - 浏览器自动化和网络测试
* [交互模式](/claude-code/08-reference/02-interactive-mode) - 快捷键、输入模式和交互功能
* [快速入门指南](/claude-code/01-getting-started/02-quickstart) - Claude Code 入门
* [常见工作流](/claude-code/02-core-concepts/03-common-workflows) - 高级工作流和模式
