...
<Note>
  对于内置命令（如 `/help` 和 `/compact`），请参阅[交互模式](/claude-code/08-reference/02-interactive-mode#built-in-commands)。
...
| 位置 | 路径                                       | 适用于        |
| :- | :--------------------------------------- | :--------- |
| 企业 | 参阅[托管设置](/claude-code/07-configuration/02-permissions#managed-settings)    | 你的组织中的所有用户 |
...
| `hooks`                    | 否  | 限定于此技能生命周期的钩子。有关配置格式，请参阅[钩子](/claude-code/04-build-with-claude/03-hooks)。                        |
...
<Note>
  在常规会话中，技能描述被加载到上下文中，以便 Claude 知道什么可用，但完整的技能内容仅在调用时加载。[预加载技能的子代理](/claude-code/04-build-with-claude/02-sub-agents#preload-skills-into-subagents)的工作方式不同：完整的技能内容在启动时注入。
</Note>
...
<Tip>
  要在技能中启用[扩展思考](/claude-code/02-core-concepts/03-common-workflows#use-extended-thinking-thinking-mode)，在你的技能内容中的任何地方包含单词"ultrathink"。
</Tip>
...
技能和[子代理](/claude-code/04-build-with-claude/02-sub-agents)在两个方向上协同工作：
...
使用 `context: fork`，你在技能中编写任务并选择代理类型来执行它。对于反向操作（定义使用技能作为参考材料的自定义子代理），请参阅[子代理](/claude-code/04-build-with-claude/02-sub-agents#preload-skills-into-subagents)。
...
**使用[权限规则](/claude-code/07-configuration/02-permissions)允许或拒绝特定技能**：
...
* **插件**：在你的[插件](/claude-code/04-build-with-claude/06-plugins)中创建 `skills/` 目录
* **托管**：通过[托管设置](/claude-code/07-configuration/02-permissions#managed-settings)部署组织范围内
...
* **[子代理](/claude-code/04-build-with-claude/02-sub-agents)**：将任务委派给专门的代理
* **[插件](/claude-code/04-build-with-claude/06-plugins)**：打包和分发技能与其他扩展
* **[钩子](/claude-code/04-build-with-claude/03-hooks)**：围绕工具事件自动化工作流
* **[内存](/claude-code/07-configuration/03-memory)**：管理 CLAUDE.md 文件以获得持久上下文
* **[交互模式](/claude-code/08-reference/02-interactive-mode#built-in-commands)**：内置命令和快捷方式
* **[权限](/claude-code/07-configuration/02-permissions)**：控制工具和技能访问
