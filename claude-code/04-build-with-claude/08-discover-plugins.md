...
想要创建和分发自己的市场？请参阅[创建和分发插件市场](/claude-code/04-build-with-claude/09-plugin-marketplaces)。
...
<Note>
  官方市场由 Anthropic 维护。要分发您自己的插件，请[创建您自己的市场](/claude-code/04-build-with-claude/09-plugin-marketplaces)并与用户共享。
</Note>
...
您也可以[为其他语言创建自己的 LSP 插件](/claude-code/04-build-with-claude/07-plugins-reference#lsp-servers)。
...
这些插件捆绑预配置的 [MCP 服务器](/claude-code/04-build-with-claude/05-mcp)，以便您可以将 Claude 连接到外部服务，无需手动设置：
...
<Steps>
  ...
  <Step title="安装插件">
    ...
    * **本地范围**：仅在此存储库中为自己安装
    ...
    请参阅[配置范围](/claude-code/07-configuration/01-settings#configuration-scopes)以了解有关范围的更多信息。
  </Step>
...
<Note>
  与基于 Git 的市场相比，基于 URL 的市场有一些限制。如果在安装插件时遇到"路径未找到"错误，请参阅[故障排除](/claude-code/04-build-with-claude/09-plugin-marketplaces#plugins-with-relative-paths-fail-in-url-based-marketplaces)。
</Note>
...
要选择不同的[安装范围](/claude-code/07-configuration/01-settings#configuration-scopes)，请使用交互式 UI：运行 `/plugin`，转到**发现**选项卡，然后在插件上按 **Enter**。您将看到以下选项：

* **用户范围**（默认）：在所有项目中为自己安装
* **项目范围**：为此存储库上的所有协作者安装（添加到 `.claude/settings.json`）
* **本地范围**：仅在此存储库中为自己安装（不与协作者共享）

您也可能看到具有**托管**范围的插件——这些由管理员通过[托管设置](/claude-code/07-configuration/01-settings#settings-files)安装，无法修改。
...
要完全禁用 Claude Code 和所有插件的所有自动更新，请设置 `DISABLE_AUTOUPDATER` 环境变量。有关详细信息，请参阅[自动更新](/claude-code/01-getting-started/03-setup#auto-updates)。
...
有关完整配置选项（包括 `extraKnownMarketplaces` 和 `enabledPlugins`），请参阅[插件设置](/claude-code/07-configuration/01-settings#plugin-settings)。
...
2. **更新 Claude Code**：
   ...
   * **本地安装程序**：从[设置](/claude-code/01-getting-started/03-setup)重新运行安装命令
...
* **插件技能未出现**：使用 `rm -rf ~/.claude/plugins/cache` 清除缓存，重启 Claude Code，然后重新安装插件。有关详细信息，请参阅[插件技能安装后未出现](/claude-code/04-build-with-claude/01-skills#plugin-skills-not-appearing-after-installation)。

有关详细的故障排除和解决方案，请参阅市场指南中的[故障排除](/claude-code/04-build-with-claude/09-plugin-marketplaces#troubleshooting)。有关调试工具，请参阅[调试和开发工具](/claude-code/04-build-with-claude/07-plugins-reference#debugging-and-development-tools)。

## 后续步骤

* **构建您自己的插件**：请参阅[插件](/claude-code/04-build-with-claude/06-plugins)以创建自定义命令、代理和钩子
* **创建市场**：请参阅[创建插件市场](/claude-code/04-build-with-claude/09-plugin-marketplaces)以将插件分发给您的团队或社区
* **技术参考**：请参阅[插件参考](/claude-code/04-build-with-claude/07-plugins-reference)以获取完整规范
