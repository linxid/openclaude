...
对于 Enterprise 和 Teams 账户，两个可见性选项是**私有**和**团队**。团队可见性使会话对你的 Claude.ai 组织的其他成员可见。默认情况下启用存储库访问验证，基于连接到收件人账户的 GitHub 账户。你的账户显示名称对所有有访问权限的收件人可见。[Claude in Slack](/claude-code/03-platforms/06-slack) 会话自动以团队可见性共享。
...
使用 [SessionStart hooks](/claude-code/04-build-with-claude/03-hooks#sessionstart) 配置自动依赖安装。这可以在你的存储库的 `.claude/settings.json` 文件中配置：
...
SessionStart hooks 可以通过写入 `CLAUDE_ENV_FILE` 环境变量中指定的文件来为后续 bash 命令持久化环境变量。有关详情，请参阅 hooks 参考中的 [SessionStart hooks](/claude-code/04-build-with-claude/03-hooks#sessionstart)。
...
1. **使用 Claude Code hooks**：配置 [SessionStart hooks](/claude-code/04-build-with-claude/03-hooks#sessionstart) 以自动化环境设置和依赖安装。
2. **记录要求**：在你的 `CLAUDE.md` 文件中清楚地指定依赖和命令。如果你有 `AGENTS.md` 文件，你可以在你的 `CLAUDE.md` 中使用 `@AGENTS.md` 来源它以维护单一的真实来源。

## 相关资源

* [Hooks 配置](/claude-code/04-build-with-claude/03-hooks)
* [设置参考](/claude-code/07-configuration/01-settings)
* [安全](/claude-code/09-resources/01-security)
* [数据使用](/claude-code/09-resources/02-data-usage)
