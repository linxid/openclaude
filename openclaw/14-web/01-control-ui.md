...
一旦批准，设备会被记住，除非你使用 `openclaw devices revoke --device <id> --role <role>` 撤销它，否则不需要重新批准。参见 [Devices CLI](/05-cli/devices) 了解 token 轮换和撤销。
...
参见 [Tailscale](/04-gateway/tailscale) 了解 HTTPS 设置指南。
...
远程访问设置详情：[远程访问](/04-gateway/remote)。
