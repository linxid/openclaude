...
* 已启用 OpenClaw hooks（参见 [Webhooks](/08-automation/webhook)）。
...
要进一步自定义负载处理，请添加 `hooks.mappings` 或在 `hooks.transformsDir` 下添加 JS/TS 转换模块（参见 [Webhooks](/08-automation/webhook)）。
...
gog gmail watch serve \
  ...
  --path /gmail-pubsub \
  --token <shared> \
  --hook-url http://127.0.0.1:18789/13-hooks/gmail \
...
* `--hook-url` 指向 OpenClaw `/13-hooks/gmail`（已映射；隔离运行 + 摘要发送到主线程）。
