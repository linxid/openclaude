...
* `openclaw webhooks gmail setup` 为 `openclaw webhooks gmail run` 写入 `hooks.gmail` 配置。完整的 Gmail 监听流程请参阅 [Gmail Pub/Sub](/08-automation/gmail-pubsub)。

## 响应

* `200` 用于 `/13-hooks/wake`
* `202` 用于 `/13-hooks/agent`（异步运行已启动）
* `401` 认证失败
* `400` 请求体无效
* `413` 请求体过大
...
```bash  theme={null}
curl -X POST http://127.0.0.1:18789/13-hooks/wake \
...
```bash  theme={null}
curl -X POST http://127.0.0.1:18789/13-hooks/agent \
...
```bash  theme={null}
curl -X POST http://127.0.0.1:18789/13-hooks/gmail \
  -H 'Authorization: Bearer SECRET' \
  -H 'Content-Type: application/json' \
