...
* **Webhooks**：外部 HTTP webhooks，让其他系统触发 OpenClaw 中的工作。参见 [Webhook Hooks](/08-automation/webhook) 或使用 `openclaw webhooks` 获取 Gmail 助手命令。

Hooks 也可以捆绑在插件中；参见 [插件](/07-tools/plugin#plugin-hooks)。
...
1. **工作区 hooks**：`<workspace>/13-hooks/`（每智能体，最高优先级）
2. **托管 hooks**：`~/.openclaw/13-hooks/`（用户安装，跨工作区共享）
3. **捆绑 hooks**：`<openclaw>/dist/13-hooks/bundled/`（随 OpenClaw 附带）
...
```json  theme={null}
{
  ...
  "version": "0.1.0",
  "openclaw": {
    "hooks": ["./13-hooks/my-hook", "./13-hooks/other-hook"]
  }
}
```

每个条目指向包含 `HOOK.md` 和 `handler.ts`（或 `index.ts`）的 hook 目录。
Hook 包可以附带依赖；它们将安装在 `~/.openclaw/13-hooks/<id>` 下。
...
import type { HookHandler } from "../../src/13-hooks/hooks.js";
...
* **`tool_result_persist`**：在工具结果写入会话记录之前转换它们。必须是同步的；返回更新后的工具结果负载或 `undefined` 保持原样。参见 [智能体循环](/03-concepts/agent-loop)。
...
* **工作区 hooks**（`<workspace>/13-hooks/`）：每智能体，最高优先级
* **托管 hooks**（`~/.openclaw/13-hooks/`）：跨工作区共享

### 2. 创建目录结构

```bash  theme={null}
mkdir -p ~/.openclaw/13-hooks/my-hook
cd ~/.openclaw/13-hooks/my-hook
```
  ...
  "hooks": {
    "internal": {
      ...
      "handlers": [
        {
          "event": "command:new",
          "module": "./13-hooks/handlers/my-handler.ts",
          "export": "default"
        }
      ]
    }
...
**文档**：[SOUL Evil Hook](/13-hooks/soul-evil)
...
import { createHookEvent } from "./src/13-hooks/hooks.js";
import myHandler from "./13-hooks/my-hook/handler.js";
...
* **`src/13-hooks/types.ts`**：类型定义
* **`src/13-hooks/workspace.ts`**：目录扫描和加载
* **`src/13-hooks/frontmatter.ts`**：HOOK.md 元数据解析
* **`src/13-hooks/config.ts`**：资格检查
* **`src/13-hooks/hooks-status.ts`**：状态报告
* **`src/13-hooks/loader.ts`**：动态模块加载器
* **`src/05-cli/hooks-cli.ts`**：CLI 命令
* **`src/04-gateway/server-startup.ts`**：在 Gateway 网关启动时加载 hooks
...
1. 检查目录结构：
   ...
   ```bash  theme={null}
   ls -la ~/.openclaw/13-hooks/my-hook/
...
2. 验证 HOOK.md 格式：
   ...
   ```bash  theme={null}
   cat ~/.openclaw/13-hooks/my-hook/HOOK.md
...
1. 创建 hook 目录：
   ...
   ```bash  theme={null}
   mkdir -p ~/.openclaw/13-hooks/my-hook
   mv ./13-hooks/handlers/my-handler.ts ~/.openclaw/13-hooks/my-hook/handler.ts
   ```
...
* [CLI 参考：hooks](/05-cli/hooks)
* [捆绑 Hooks README](https://github.com/openclaw/openclaw/tree/main/src/13-hooks/bundled)
* [Webhook Hooks](/08-automation/webhook)
* [配置](/04-gateway/configuration#hooks)
