...
有关规范的 JSON 结构和示例，请参阅[工具调用的 JSON 模式](/08-automation/cron-jobs#json-schema-for-tool-calls)。
...
当你需要正常的心跳提示 + 主会话上下文时，这是最佳选择。参见[心跳](/04-gateway/heartbeat)。
...
* `cron.run`（强制或到期）、`cron.runs`
  如需不创建任务直接发送系统事件，请使用 [`openclaw system event`](/05-cli/system)。
