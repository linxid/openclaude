...
**节点**是一个配套设备（macOS/iOS/Android/无头），它以 `role: "node"` 连接到 Gateway 网关 **WebSocket**（与操作员相同的端口），并通过 `node.invoke` 暴露命令接口（例如 `canvas.*`、`camera.*`、`system.*`）。协议详情：[Gateway 网关协议](/openclaw/gateway/protocol)。

旧版传输：[Bridge 协议](/openclaw/gateway/bridge-protocol)（TCP JSONL；当前节点已弃用/移除）。
...
* [节点主机 CLI](/openclaw/cli/node)
* [Exec 工具](/openclaw/tools/exec)
* [Exec 批准](/openclaw/tools/exec-approvals)
...
* Exec 批准通过 `~/.openclaw/exec-approvals.json` 在本地执行
  （参见 [Exec 批准](/openclaw/tools/exec-approvals)）。
