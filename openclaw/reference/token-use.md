...
完整分解参见[系统提示词](/openclaw/concepts/system-prompt)。
...
有关实际分解（每个注入文件、工具、Skills 和系统提示词大小），使用 `/context list` 或 `/context detail`。参见[上下文](/openclaw/concepts/context)。
...
在 [Gateway 网关配置](/openclaw/gateway/configuration) 中配置它，并在
[会话修剪](/openclaw/concepts/session-pruning) 中查看行为详情。
...
精确的 skill 列表开销公式参见 [Skills](/openclaw/tools/skills)。
