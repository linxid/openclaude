...
* 阅读本文档和 `docs/10-platforms/mac/release.md`。
...
* [ ] 更新 CLI/版本字符串：[`src/05-cli/program.ts`](https://github.com/openclaw/openclaw/blob/main/src/05-cli/program.ts) 和 [`src/provider-web.ts`](https://github.com/openclaw/openclaw/blob/main/src/provider-web.ts) 中的 Baileys user agent。
...
* [ ] 按照 [macOS 发布](/10-platforms/mac/release) 获取确切命令和所需环境变量。
  * `APP_BUILD` 必须是数字且单调递增（不带 `-beta`），以便 Sparkle 正确比较版本。
  * 如果进行公证，使用从 App Store Connect API 环境变量创建的 `openclaw-notary` 钥匙串配置文件（参见 [macOS 发布](/10-platforms/mac/release)）。
