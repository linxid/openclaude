...
* [/03-concepts/session](/03-concepts/session)
* [/03-concepts/compaction](/03-concepts/compaction)
* [/03-concepts/session-pruning](/03-concepts/session-pruning)
* [/17-reference/transcript-hygiene](/17-reference/transcript-hygiene)
...
规范规则记录在 [/03-concepts/session](/03-concepts/session)。
...
更多信息，参见 [/token-use](/17-reference/token-use)。
...
压缩是**持久化的**（与会话修剪不同）。参见 [/03-concepts/session-pruning](/03-concepts/session-pruning)。
...
* 参见[记忆](/03-concepts/memory)了解工作空间文件布局和写入模式。
...
* 会话键错误？从 [/03-concepts/session](/03-concepts/session) 开始，并在 `/status` 中确认 `sessionKey`。
