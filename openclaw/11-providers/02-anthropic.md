...
如果你覆盖提供商头信息，请保留它（参见 [/04-gateway/configuration](/04-gateway/configuration)）。
...
* 如果你在 Claude 订阅上看到"OAuth token refresh failed …"，请使用 setup-token 重新认证。参见 [/04-gateway/troubleshooting#oauth-token-refresh-failed-anthropic-claude-subscription](/04-gateway/troubleshooting#oauth-token-refresh-failed-anthropic-claude-subscription)。
* 认证详情 + 重用规则在 [/03-concepts/oauth](/03-concepts/oauth)。
...
更多信息：[/04-gateway/troubleshooting](/04-gateway/troubleshooting) 和 [/18-help/faq](/18-help/faq)。
