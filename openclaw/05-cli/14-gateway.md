...
* [/04-gateway/bonjour](/04-gateway/bonjour)
* [/04-gateway/discovery](/04-gateway/discovery)
* [/04-gateway/configuration](/04-gateway/configuration)
...
* 组播 DNS-SD：`local.`
* 单播 DNS-SD（广域 Bonjour）：选择一个域（示例：`openclaw.internal.`）并设置分割 DNS + DNS 服务器；参见 [/04-gateway/bonjour](/04-gateway/bonjour)
