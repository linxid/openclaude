...
可以，通过 `bindings` 将每个发送者路由到不同的智能体（peer `kind: "dm"`，发送者 E.164 如 `+15551234567`）。回复仍然来自**同一个 WhatsApp 账户**，直接聊天会折叠到每个智能体的主会话，因此**每人使用一个智能体**。私信访问控制（`dmPolicy`/`allowFrom`）是每个 WhatsApp 账户全局的。参见[多智能体路由](/03-concepts/multi-agent)。
...
* 表情移除语义：参见 [/07-tools/reactions](/07-tools/reactions)。
...
* 故障排除指南：[Gateway 网关故障排除](/04-gateway/troubleshooting)。
