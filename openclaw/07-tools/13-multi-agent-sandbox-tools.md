...
有关沙箱隔离在运行时的行为，请参见[沙箱隔离](/04-gateway/sandboxing)。
有关调试"为什么这被阻止了？"，请参见[沙箱 vs 工具策略 vs 提权](/04-gateway/sandbox-vs-tool-policy-vs-elevated) 和 `openclaw sandbox explain`。
