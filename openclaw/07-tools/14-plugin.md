...
参见 [Voice Call](/12-plugins/voice-call) 了解具体的插件示例。
...
* [Voice Call](/12-plugins/voice-call) — `@openclaw/voice-call`
* [Zalo Personal](/12-plugins/zalouser) — `@openclaw/zalouser`
* [Matrix](/06-channels/matrix) — `@openclaw/matrix`
* [Nostr](/06-channels/nostr) — `@openclaw/nostr`
* [Zalo](/06-channels/zalo) — `@openclaw/zalo`
* [Microsoft Teams](/06-channels/msteams) — `@openclaw/msteams`
...
OpenClaw 插件是通过 jiti 在运行时加载的 **TypeScript 模块**。**配置验证不会执行插件代码**；它使用插件清单和 JSON Schema。参见 [插件清单](/12-plugins/manifest)。
...
工具编写指南：[插件智能体工具](/12-plugins/agent-tools)。
...
```json  theme={null}
{
  ...
  "openclaw": {
    ...
    "channel": {
      ...
      "docsPath": "/06-channels/nextcloud-talk",
...
* `~/.openclaw/12-plugins/catalog.json`
...
* `meta.docsPath` 应指向像 `/06-channels/<id>` 这样的文档页面。
...
参见专门指南：[插件智能体工具](/12-plugins/agent-tools)。
...
参见 [Voice Call](/12-plugins/voice-call) 和 `extensions/voice-call/README.md` 了解设置和用法。
...
* 仓库内插件可以在 `src/**` 下保留 Vitest 测试（例如：`src/12-plugins/voice-call.plugin.test.ts`）。
