...
* **沙箱注意事项**：智能体沙箱隔离也使用 Docker，但它**不需要**完整的 Gateway 网关在 Docker 中运行。参阅[沙箱隔离](/04-gateway/sandboxing)。
...
沙箱隔离详情：[沙箱隔离](/04-gateway/sandboxing)
...
在 VPS 上运行？参阅 [Hetzner（Docker VPS）](/02-install/hetzner)。
...
更多详情：[仪表板](/14-web/dashboard)，[设备](/05-cli/devices)。
...
文档：[WhatsApp](/06-channels/whatsapp)，[Telegram](/06-channels/telegram)，[Discord](/06-channels/discord)
...
深入了解：[沙箱隔离](/04-gateway/sandboxing)
...
参阅[多智能体沙箱与工具](/07-tools/multi-agent-sandbox-tools)了解示例、优先级和故障排除。
