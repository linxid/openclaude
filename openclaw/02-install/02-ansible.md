...
注意：Gateway 网关**直接在主机上运行**（不在 Docker 中），但智能体沙箱使用 Docker 进行隔离。详情参见[沙箱隔离](/04-gateway/sandboxing)。
...
沙箱配置参见[多智能体沙箱与工具](/07-tools/multi-agent-sandbox-tools)。
...
Ansible 安装程序设置 OpenClaw 为手动更新。标准更新流程参见[更新](/02-install/updating)。
...
* [Docker](/02-install/docker) — 容器化 Gateway 网关设置
* [沙箱隔离](/04-gateway/sandboxing) — 智能体沙箱配置
* [多智能体沙箱与工具](/07-tools/multi-agent-sandbox-tools) — 每个智能体的隔离
