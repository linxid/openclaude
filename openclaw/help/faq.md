...
快速解答及针对实际部署场景（本地开发、VPS、多智能体、OAuth/API 密钥、模型故障转移）的深入故障排除。运行时诊断请参阅[故障排除](/openclaw/gateway/troubleshooting)。完整配置参考请参阅[配置](/openclaw/gateway/configuration)。
...
4. **深度探测**
   ...
   运行 Gateway 网关健康检查 + 提供商探测（需要可达的 Gateway 网关）。参阅[健康检查](/openclaw/gateway/health)。
...
5. **跟踪最新日志**
   ...
   文件日志与服务日志是分开的；参阅[日志](/logging)和[故障排除](/openclaw/gateway/troubleshooting)。
...
6. **运行 doctor（修复）**
   ...
   修复/迁移配置/状态 + 运行健康检查。参阅 [Doctor](/openclaw/gateway/doctor)。
...
7. **Gateway 网关快照**
   ...
   ```
   向运行中的 Gateway 网关请求完整快照（仅 WS）。参阅[健康检查](/openclaw/gateway/health)。
...
安装文档：[安装](/install)、[安装程序标志](/openclaw/install/installer)、[更新](/openclaw/install/updating)。
...
参阅[仪表板](/openclaw/web/dashboard)和 [Web 界面](/web)了解绑定模式和认证详情。
...
文档：[Linux](/openclaw/platforms/linux)、[安装](/install)。
...
如果 Gateway 网关在远程，确保隧道/Tailscale 连接正常，且 UI 指向正确的 Gateway 网关。参阅[远程访问](/openclaw/gateway/remote)。
...
相关：[迁移](/openclaw/install/migrating)、[磁盘上的文件位置](/openclaw/help/faq#where-does-openclaw-store-its-data)、
[智能体工作区](/openclaw/concepts/agent-workspace)、[Doctor](/openclaw/gateway/doctor)、
[远程模式](/openclaw/gateway/remote)。
...
一些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 错误地拦截了 `docs.openclaw.ai`。禁用该功能或将 `docs.openclaw.ai` 加入白名单，然后重试。更多详情：[故障排除](/openclaw/help/troubleshooting#docsopenclawai-shows-an-ssl-error-comcastxfinity)。
...
更多详情：[开发渠道](/openclaw/install/development-channels)和[安装程序标志](/openclaw/install/installer)。
...
如果挂起，请参阅[安装程序卡住](/openclaw/help/faq#installer-stuck-how-do-i-get-more-feedback)和[我卡住了](/openclaw/help/faq#im-stuck--whats-the-fastest-way-to-get-unstuck)中的快速调试流程。
...
文档：[更新](/openclaw/cli/update)、[开发渠道](/openclaw/install/development-channels)、
[安装](/install)。
...
更多选项：[安装程序标志](/openclaw/install/installer)。
...
文档：[Windows](/openclaw/platforms/windows)。
...
更多详情：[安装](/install)和[安装程序标志](/openclaw/install/installer)。

### 如何在 Linux 上安装 OpenClaw

简短回答：按照 Linux 指南操作，然后运行新手引导向导。

* Linux 快速路径 + 服务安装：[Linux](/openclaw/platforms/linux)。
* 完整指南：[入门](/openclaw/start/getting-started)。
* 安装和更新：[安装与更新](/openclaw/install/updating)。

### 如何在 VPS 上安装 OpenClaw

任何 Linux VPS 都可以。在服务器上安装，然后使用 SSH/Tailscale 访问 Gateway 网关。

指南：[exe.dev](/openclaw/install/exe-dev)、[Hetzner](/openclaw/install/hetzner)、[Fly.io](/openclaw/install/fly)。
远程访问：[Gateway 网关远程](/openclaw/gateway/remote)。
...
* [VPS 托管](/vps)（所有提供商汇总）
* [Fly.io](/openclaw/install/fly)
* [Hetzner](/openclaw/install/hetzner)
* [exe.dev](/openclaw/install/exe-dev)
...
中心：[平台](/platforms)。远程访问：[Gateway 网关远程](/openclaw/gateway/remote)。
节点：[节点](/nodes)、[节点 CLI](/openclaw/cli/nodes)。
...
文档：[更新](/openclaw/cli/update)、[更新指南](/openclaw/install/updating)。
...
文档：[Anthropic](/openclaw/providers/anthropic)、[OpenAI](/openclaw/providers/openai)、
[本地模型](/openclaw/gateway/local-models)、[模型](/openclaw/concepts/models)。
...
`claude setup-token` 通过 Claude Code CLI 生成一个**令牌字符串**（在 Web 控制台中不可用）。你可以在**任何机器**上运行它。在向导中选择 **Anthropic token (paste setup-token)** 或使用 `openclaw models auth paste-token --provider anthropic` 粘贴。令牌作为 **anthropic** 提供商的认证配置文件存储，像 API 密钥一样使用（无自动刷新）。更多详情：[OAuth](/openclaw/concepts/oauth)。
...
复制它打印的令牌，然后在向导中选择 **Anthropic token (paste setup-token)**。如果你想在 Gateway 网关主机上运行，使用 `openclaw models auth setup-token --provider anthropic`。如果你在其他地方运行了 `claude setup-token`，在 Gateway 网关主机上使用 `openclaw models auth paste-token --provider anthropic` 粘贴。参阅 [Anthropic](/openclaw/providers/anthropic)。

### 是否支持 Claude 订阅认证（Claude Pro/Max）

是的——通过 **setup-token**。OpenClaw 不再复用 Claude Code CLI OAuth 令牌；请使用 setup-token 或 Anthropic API 密钥。在任何地方生成令牌并在 Gateway 网关主机上粘贴。参阅 [Anthropic](/openclaw/providers/anthropic) 和 [OAuth](/openclaw/concepts/oauth)。
...
参阅[模型](/openclaw/cli/models)和 [OAuth](/openclaw/concepts/oauth)。

### 支持 AWS Bedrock 吗

是的——通过 pi-ai 的 **Amazon Bedrock (Converse)** 提供商进行**手动配置**。你必须在 Gateway 网关主机上提供 AWS 凭据/区域，并在模型配置中添加 Bedrock 提供商条目。参阅 [Amazon Bedrock](/openclaw/providers/bedrock) 和[模型提供商](/openclaw/providers/models)。如果你更喜欢托管密钥流程，在 Bedrock 前面使用兼容 OpenAI 的代理仍然是有效选项。

### Codex 认证如何工作

OpenClaw 通过 OAuth（ChatGPT 登录）支持 **OpenAI Code (Codex)**。向导可以运行 OAuth 流程，并在适当时将默认模型设置为 `openai-codex/gpt-5.2`。参阅[模型提供商](/openclaw/concepts/model-providers)和[向导](/openclaw/start/wizard)。

### 是否支持 OpenAI 订阅认证（Codex OAuth）

是的。OpenClaw 完全支持 **OpenAI Code (Codex) 订阅 OAuth**。新手引导向导可以为你运行 OAuth 流程。

参阅 [OAuth](/openclaw/concepts/oauth)、[模型提供商](/openclaw/concepts/model-providers)和[向导](/openclaw/start/wizard)。
...
这会在 Gateway 网关主机上将 OAuth 令牌存储为认证配置文件。详情：[模型提供商](/openclaw/concepts/model-providers)。

### 本地模型适合日常聊天吗

通常不适合。OpenClaw 需要大上下文 + 强安全性；小显卡会截断且泄漏。如果必须使用，请在本地运行你能运行的**最大** MiniMax M2.1 版本（LM Studio），参阅 [/gateway/local-models](/openclaw/gateway/local-models)。较小/量化的模型会增加提示注入风险——参阅[安全](/openclaw/gateway/security)。
...
文档：[iMessage](/openclaw/channels/imessage)、[节点](/nodes)、[Mac 远程模式](/openclaw/platforms/mac/remote)。
...
文档：[iMessage](/openclaw/channels/imessage)、[BlueBubbles](/openclaw/channels/bluebubbles)、
[Mac 远程模式](/openclaw/platforms/mac/remote)。
...
文档：[节点](/nodes)、[节点 CLI](/openclaw/cli/nodes)。
...
参阅 [/channels/telegram](/openclaw/channels/telegram#access-control-dms--groups)。

### 多人能否使用同一个 WhatsApp 号码配合不同的 OpenClaw 实例

可以，通过**多智能体路由**。将每个发送者的 WhatsApp **私信**（peer `kind: "dm"`，发送者 E.164 格式如 `+15551234567`）绑定到不同的 `agentId`，这样每个人获得自己的工作区和会话存储。回复仍然来自**同一个 WhatsApp 账户**，且私信访问控制（`channels.whatsapp.dmPolicy` / `channels.whatsapp.allowFrom`）对每个 WhatsApp 账户是全局的。参阅[多智能体路由](/openclaw/concepts/multi-agent)和 [WhatsApp](/openclaw/channels/whatsapp)。

### 能否同时运行一个“快速聊天”智能体和一个“用 Opus 编程”的智能体

可以。使用多智能体路由：为每个智能体设置自己的默认模型，然后将入站路由（提供商账户或特定对等方）绑定到每个智能体。示例配置位于[多智能体路由](/openclaw/concepts/multi-agent)。另参阅[模型](/openclaw/concepts/models)和[配置](/openclaw/gateway/configuration)。
...
文档：[入门](/openclaw/start/getting-started)、[更新](/openclaw/install/updating)。
...
备份提示：参阅[备份策略](/openclaw/help/faq#whats-the-recommended-backup-strategy)。
...
**OpenClaw 特定说明：** WhatsApp/Telegram/Slack/Mattermost（插件）/Discord 在 VPS 上都能正常工作。唯一的真正权衡是**无头浏览器**与可见窗口。参阅[浏览器](/openclaw/tools/browser)。
...
安全指南请阅读[安全](/openclaw/gateway/security)。
...
文档：[Linux](/openclaw/platforms/linux)、[VPS 托管](/vps)。
...
如果你使用 Windows，**WSL2 是最简单的虚拟机式设置**，具有最佳的工具兼容性。参阅 [Windows](/openclaw/platforms/windows)、[VPS 托管](/vps)。
如果你在虚拟机中运行 macOS，参阅 [macOS VM](/openclaw/install/macos-vm)。
...
文档：[Gateway 网关](/gateway)、[渠道](/channels)、[多智能体](/openclaw/concepts/multi-agent)、
...
文档：[安全](/openclaw/gateway/security)。
...
参阅[定时任务](/automation/cron-jobs)、[多智能体路由](/openclaw/concepts/multi-agent)和[斜杠命令](/openclaw/tools/slash-commands)。
...
文档：[子智能体](/openclaw/tools/subagents)。
...
对于 iMessage，你也可以将 `channels.imessage.cliPath` 指向 SSH 包装器（OpenClaw 只需要 stdio）。参阅 [iMessage](/openclaw/channels/imessage)。
...
ClawHub 安装到当前目录下的 `./skills`（或回退到你配置的 OpenClaw 工作区）；OpenClaw 在下一个会话中将其视为 `<workspace>/skills`。对于跨智能体共享的 Skills，将它们放在 `~/.openclaw/skills/<name>/SKILL.md`。某些 Skills 期望通过 Homebrew 安装二进制文件；在 Linux 上意味着 Linuxbrew（参阅上面的 Homebrew Linux 常见问题条目）。参阅[Skills](/openclaw/tools/skills)和 [ClawHub](/openclaw/tools/clawhub)。
...
完整指南（包括远程 Gateway 网关 + 安全注意事项）：[Chrome 扩展](/openclaw/tools/chrome-extension)
...
有。参阅[沙箱](/openclaw/gateway/sandboxing)。对于 Docker 特定设置（完整 Gateway 网关在 Docker 中或沙箱镜像），参阅 [Docker](/openclaw/install/docker)。
...
设置指南 + 示例配置：[群组：个人私信 + 公开群组](/openclaw/channels/groups#pattern-personal-dms-public-groups-single-agent)

关键配置参考：[Gateway 网关配置](/openclaw/gateway/configuration#agentsdefaultssandbox)

### 如何将主机文件夹绑定到沙箱中

将 `agents.defaults.sandbox.docker.binds` 设置为 `["host:path:mode"]`（例如 `"/home/user/src:/src:ro"`）。全局 + 按智能体的绑定会合并；当 `scope: "shared"` 时按智能体的绑定会被忽略。对于敏感内容使用 `:ro`，并记住绑定会绕过沙箱文件系统隔离。参阅[沙箱](/openclaw/gateway/sandboxing#custom-bind-mounts)和[沙箱 vs 工具策略 vs 提权](/openclaw/gateway/sandbox-vs-tool-policy-vs-elevated#bind-mounts-security-quick-check)了解示例和安全注意事项。
...
OpenClaw 还会运行**静默的预压缩记忆刷新**，以提醒模型在自动压缩之前写入持久笔记。这只在工作区可写时运行（只读沙箱会跳过）。参阅[记忆](/openclaw/concepts/memory)。
...
文档：[记忆](/openclaw/concepts/memory)、[智能体工作区](/openclaw/concepts/agent-workspace)。
...
如果你更想保持本地运行，设置 `memorySearch.provider = "local"`（可选 `memorySearch.fallback = "none"`）。如果你想使用 Gemini embeddings，设置 `memorySearch.provider = "gemini"` 并提供 `GEMINI_API_KEY`（或 `memorySearch.remote.apiKey`）。我们支持 **OpenAI、Gemini 或本地** embedding 模型——参阅[记忆](/openclaw/concepts/memory)了解设置详情。

### 记忆是否永久保留？有什么限制

记忆文件保存在磁盘上，持久存在直到你删除它们。限制是你的存储空间，而不是模型。**会话上下文**仍然受模型上下文窗口限制，所以长对话可能会压缩或截断。这就是记忆搜索存在的原因——它只将相关部分拉回上下文。

文档：[记忆](/openclaw/concepts/memory)、[上下文](/openclaw/concepts/context)。
...
相关：[智能体工作区](/openclaw/concepts/agent-workspace)、[记忆](/openclaw/concepts/memory)。
...
参阅[智能体工作区](/openclaw/concepts/agent-workspace)和[记忆](/openclaw/concepts/memory)。
...
文档：[智能体工作区](/openclaw/concepts/agent-workspace)。

### 如何完全卸载 OpenClaw

参阅专门指南：[卸载](/openclaw/install/uninstall)。

### 智能体可以在工作区外工作吗

可以。工作区是**默认 cwd** 和记忆锚点，不是硬沙箱。相对路径在工作区内解析，但绝对路径可以访问其他主机位置，除非启用了沙箱。如果你需要隔离，使用 [`agents.defaults.sandbox`](/openclaw/gateway/sandboxing) 或按智能体的沙箱设置。如果你希望某个仓库作为默认工作目录，将该智能体的 `workspace` 指向仓库根目录。OpenClaw 仓库只是源代码；除非你有意要让智能体在其中工作，否则保持工作区独立。
...
会话状态归 **Gateway 网关主机**所有。如果你处于远程模式，你关心的会话存储在远程机器上，而不是你的本地笔记本上。参阅[会话管理](/openclaw/concepts/session)。
...
文档：[Config](/openclaw/cli/config)、[Configure](/openclaw/cli/configure)、[Doctor](/openclaw/gateway/doctor)。
...
文档：[节点](/nodes)、[远程访问](/openclaw/gateway/remote)、[多智能体路由](/openclaw/concepts/multi-agent)、[子智能体](/openclaw/tools/subagents)、[TUI](/openclaw/web/tui)。
...
默认为 `false`（有头）。无头模式在某些网站上更容易触发反机器人检测。参阅[浏览器](/openclaw/tools/browser)。
...
参阅[浏览器](/openclaw/tools/browser#use-brave-or-another-chromium-based-browser)中的完整配置示例。
...
安全提醒：配对 macOS 节点允许在该机器上执行 `system.run`。只配对你信任的设备，并查阅[安全](/openclaw/gateway/security)。

文档：[节点](/nodes)、[Gateway 网关协议](/openclaw/gateway/protocol)、[macOS 远程模式](/openclaw/platforms/mac/remote)、[安全](/openclaw/gateway/security)。
...
文档：[Tailscale](/openclaw/gateway/tailscale)、[远程访问](/openclaw/gateway/remote)、[渠道](/channels)。
...
**CLI 桥接（通用）：** 运行一个脚本调用另一个 Gateway 网关，使用 `openclaw agent --message ... --deliver`，定向到另一个机器人监听的聊天。如果一个机器人在远程 VPS 上，通过 SSH/Tailscale 将你的 CLI 指向该远程 Gateway 网关（参阅[远程访问](/openclaw/gateway/remote)）。
...
文档：[远程访问](/openclaw/gateway/remote)、[Agent CLI](/openclaw/cli/agent)、[Agent send](/openclaw/tools/agent-send)。
...
文档：[节点](/nodes)、[节点 CLI](/openclaw/cli/nodes)、[Chrome 扩展](/openclaw/tools/chrome-extension)。
...
文档：[节点](/nodes)、[节点 CLI](/openclaw/cli/nodes)、[多 Gateway 网关](/openclaw/gateway/multiple-gateways)。

### 节点会运行 Gateway 网关服务吗

不会。每台主机上应该只运行**一个 Gateway 网关**，除非你有意运行隔离的配置文件（参阅[多 Gateway 网关](/openclaw/gateway/multiple-gateways)）。节点是连接到 Gateway 网关的外围设备（iOS/Android 节点，或 macOS 菜单栏应用的“节点模式”）。对于无头节点主机和 CLI 控制，参阅[节点主机 CLI](/openclaw/cli/node)。
...
这保持 Gateway 网关绑定到 local loopback 并通过 Tailscale 暴露 HTTPS。参阅 [Tailscale](/openclaw/gateway/tailscale)。
...
文档：[Gateway 网关协议](/openclaw/gateway/protocol)、[发现](/openclaw/gateway/discovery)、[macOS 远程模式](/openclaw/platforms/mac/remote)。
...
参阅 [/environment](/openclaw/help/environment) 了解优先级和来源详情。
...
参阅 [/concepts/model-providers](/openclaw/concepts/model-providers) 和 [/environment](/openclaw/help/environment)。

## 会话与多聊天

### 如何开始一个新对话

发送 `/new` 或 `/reset` 作为独立消息。参阅[会话管理](/openclaw/concepts/session)。
...
文档：[多智能体路由](/openclaw/concepts/multi-agent)、[子智能体](/openclaw/tools/subagents)、[智能体 CLI](/openclaw/cli/agents)。
...
* 新手引导向导在看到现有配置时也提供**重置**选项。参阅[向导](/openclaw/start/wizard)。
...
文档：[压缩](/openclaw/concepts/compaction)、[会话修剪](/openclaw/concepts/session-pruning)、[会话管理](/openclaw/concepts/session)。
...
按智能体覆盖使用 `agents.list[].heartbeat`。文档：[心跳](/openclaw/gateway/heartbeat)。
...
文档：[WhatsApp](/openclaw/channels/whatsapp)、[目录](/openclaw/cli/directory)、[日志](/openclaw/cli/logs)。
...
参阅[群组](/openclaw/channels/groups)和[群组消息](/openclaw/channels/group-messages)。

### 群组/线程与私聊共享上下文吗

直接聊天默认折叠到主会话。群组/频道有自己的会话键，Telegram 话题 / Discord 线程是独立的会话。参阅[群组](/openclaw/channels/groups)和[群组消息](/openclaw/channels/group-messages)。
...
文档：[多智能体路由](/openclaw/concepts/multi-agent)、[Slack](/openclaw/channels/slack)、
[浏览器](/openclaw/tools/browser)、[Chrome 扩展](/openclaw/tools/chrome-extension)、[节点](/nodes)。
...
MiniMax M2.1 有自己的文档：[MiniMax](/openclaw/providers/minimax) 和
[本地模型](/openclaw/gateway/local-models)。

经验法则：高风险工作使用你能负担的**最好的模型**，日常聊天或摘要使用更便宜的模型。你可以按智能体路由模型并使用子智能体并行处理长任务（每个子智能体消耗令牌）。参阅[模型](/openclaw/concepts/models)和[子智能体](/openclaw/tools/subagents)。

重要警告：较弱/过度量化的模型更容易受到提示注入和不安全行为的影响。参阅[安全](/openclaw/gateway/security)。

更多上下文：[模型](/openclaw/concepts/models)。
...
文档：[Ollama](/openclaw/providers/ollama)、[本地模型](/openclaw/gateway/local-models)、
[模型提供商](/openclaw/concepts/model-providers)、[安全](/openclaw/gateway/security)、
[沙箱](/openclaw/gateway/sandboxing)。
...
文档：[模型](/openclaw/concepts/models)、[Configure](/openclaw/cli/configure)、[Config](/openclaw/cli/config)、[Doctor](/openclaw/gateway/doctor)。

### OpenClaw、Flawd 和 Krill 使用什么模型

* **OpenClaw + Flawd：** Anthropic Opus（`anthropic/claude-opus-4-5`）——参阅 [Anthropic](/openclaw/providers/anthropic)。
* **Krill：** MiniMax M2.1（`minimax/MiniMax-M2.1`）——参阅 [MiniMax](/openclaw/providers/minimax)。
...
参阅[模型](/openclaw/concepts/models)和[斜杠命令](/openclaw/tools/slash-commands)。
...
参阅 [MiniMax](/openclaw/providers/minimax) 和[模型](/openclaw/concepts/models)。
...
文档：[模型](/openclaw/concepts/models)、[多智能体路由](/openclaw/concepts/multi-agent)、[MiniMax](/openclaw/providers/minimax)、[OpenAI](/openclaw/providers/openai)。
...
相关：[/concepts/oauth](/openclaw/concepts/oauth)（OAuth 流程、令牌存储、多账户模式）
...
* 仍然卡住？运行 `openclaw status --all` 并按[故障排除](/openclaw/gateway/troubleshooting)操作。参阅[仪表板](/openclaw/web/dashboard)了解认证详情。
...
完整指南：[多 Gateway 网关](/openclaw/gateway/multiple-gateways)。
...
协议详情：[Gateway 网关协议](/openclaw/gateway/protocol)。
...
参阅[故障排除](/openclaw/gateway/troubleshooting#log-locations)了解更多。
...
文档：[Windows (WSL2)](/openclaw/platforms/windows)、[Gateway 网关服务运维手册](/gateway)。
...
文档：[渠道](/channels)、[故障排除](/openclaw/gateway/troubleshooting)、[远程访问](/openclaw/gateway/remote)。
...
文档：[仪表板](/openclaw/web/dashboard)、[远程访问](/openclaw/gateway/remote)、[故障排除](/openclaw/gateway/troubleshooting)。
...
文档：[Telegram](/openclaw/channels/telegram)、[渠道故障排除](/openclaw/channels/troubleshooting)。
...
文档：[TUI](/openclaw/web/tui)、[斜杠命令](/openclaw/tools/slash-commands)。
...
智能体的出站附件必须包含 `MEDIA:<path-or-url>` 行（独占一行）。参阅 [OpenClaw 助手设置](/openclaw/start/openclaw)和 [Agent send](/openclaw/tools/agent-send)。
...
详情：[安全](/openclaw/gateway/security)。
...
文档：[安全](/openclaw/gateway/security)、[配对](/openclaw/channels/pairing)。
...
如果你想实验，在专用账户上进行并保持隔离。参阅[安全](/openclaw/gateway/security)。

### 个人助理任务可以使用更便宜的模型吗

可以，**如果**智能体仅用于聊天且输入是可信的。较小的模型更容易受到指令劫持，因此避免将它们用于启用工具的智能体或读取不可信内容时。如果你必须使用较小的模型，锁定工具并在沙箱中运行。参阅[安全](/openclaw/gateway/security)。
...
文档：[思考和详细输出](/openclaw/tools/thinking)、[安全](/openclaw/gateway/security#reasoning--verbose-output-in-groups)。
...
斜杠命令概览：参阅[斜杠命令](/openclaw/tools/slash-commands)。
