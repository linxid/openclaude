...
参见 [Bonjour](/04-gateway/bonjour) 了解 CoreDNS 示例。
...
* `A2UI_HOST_NOT_CONFIGURED`：Gateway 网关未广播 canvas 主机 URL；检查 [Gateway 网关配置](/04-gateway/configuration) 中的 `canvasHost`。
* 配对提示从未出现：运行 `openclaw nodes pending` 并手动批准。
* 重新安装后重连失败：钥匙串配对令牌已被清除；重新配对节点。

## 相关文档

* [配对](/04-gateway/pairing)
* [设备发现](/04-gateway/discovery)
