...
* 测试：`src/04-gateway/gateway-models.profiles.live.test.ts`
* 目标：
  * 启动一个进程内 Gateway 网关
...
* 探测详情（以便你能快速解释故障）：
  ...
  * 实现参考：`src/04-gateway/gateway-models.profiles.live.test.ts` 和 `src/04-gateway/live-image-probe.ts`。
* 如何启用：
  * `pnpm test:live`（或直接调用 Vitest 时使用 `OPENCLAW_LIVE_TEST=1`）
* 如何选择模型：
...
* 工具 + 图像探测在此实时测试中始终开启：
  ...
  * 当模型声明支持图像输入时运行图像探测
  * 流程（高层次）：
    * 测试生成一个带有"CAT"+ 随机代码的小型 PNG（`src/04-gateway/live-image-probe.ts`）
    * 通过 `agent` `attachments: [{ mimeType: "image/png", content: "<base64>" }]` 发送
    * Gateway 网关将附件解析为 `images[]`（`src/04-gateway/server-methods/agent.ts` + `src/04-gateway/chat-attachments.ts`）
    * 嵌入式智能体将多模态用户消息转发给模型
...
* 测试：`src/04-gateway/gateway-cli-backend.live.test.ts`
...
OPENCLAW_LIVE_CLI_BACKEND=1 \
  ...
  pnpm test:live src/04-gateway/gateway-cli-backend.live.test.ts
```
...
* 单个模型，Gateway 网关冒烟测试：
  * `OPENCLAW_LIVE_GATEWAY_MODELS="openai/gpt-5.2" pnpm test:live src/04-gateway/gateway-models.profiles.live.test.ts`

* 跨多个提供商的工具调用：
  * `OPENCLAW_LIVE_GATEWAY_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-5,google/gemini-3-flash-preview,zai/glm-4.7,minimax/minimax-m2.1" pnpm test:live src/04-gateway/gateway-models.profiles.live.test.ts`

* Google 专项（Gemini API 密钥 + Antigravity）：
  * Gemini（API 密钥）：`OPENCLAW_LIVE_GATEWAY_MODELS="google/gemini-3-flash-preview" pnpm test:live src/04-gateway/gateway-models.profiles.live.test.ts`
  * Antigravity（OAuth）：`OPENCLAW_LIVE_GATEWAY_MODELS="google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-pro-high" pnpm test:live src/04-gateway/gateway-models.profiles.live.test.ts`
...
`OPENCLAW_LIVE_GATEWAY_MODELS="openai/gpt-5.2,openai-codex/gpt-5.2,anthropic/claude-opus-4-5,google/gemini-3-pro-preview,google/gemini-3-flash-preview,google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-flash,zai/glm-4.7,minimax/minimax-m2.1" pnpm test:live src/04-gateway/gateway-models.profiles.live.test.ts`
...
* 测试：`src/media-understanding/11-providers/deepgram/audio.live.test.ts`
* 启用：`DEEPGRAM_API_KEY=... DEEPGRAM_LIVE_TEST=1 pnpm test:live src/media-understanding/11-providers/deepgram/audio.live.test.ts`
...
* Gateway 网关工具调用（模拟 OpenAI，真实 Gateway 网关 + 智能体循环）：`src/04-gateway/gateway.tool-calling.mock-openai.test.ts`
* Gateway 网关向导（WS `wizard.start`/`wizard.next`，写入配置 + 强制认证）：`src/04-gateway/gateway.wizard.e2e.test.ts`

## 智能体可靠性评估（Skills）

我们已经有一些 CI 安全的测试，它们的行为类似于"智能体可靠性评估"：

* 通过真实 Gateway 网关 + 智能体循环的模拟工具调用（`src/04-gateway/gateway.tool-calling.mock-openai.test.ts`）。
* 验证会话连接和配置效果的端到端向导流程（`src/04-gateway/gateway.wizard.e2e.test.ts`）。

对于 Skills 仍然缺少的内容（参见 [Skills](/07-tools/skills)）：
