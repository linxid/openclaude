...
参见[插件](/07-tools/plugin)了解安装 + 配置，以及 [Skills](/07-tools/skills) 了解
工具使用指导如何被注入到提示中。一些插件随工具一起提供自己的 Skills
（例如，voice-call 插件）。

可选的插件工具：

* [Lobster](/07-tools/lobster)：带有可恢复审批的类型化工作流运行时（需要 Gateway 网关主机上的 Lobster CLI）。
* [LLM Task](/07-tools/llm-task)：用于结构化工作流输出的 JSON-only LLM 步骤（可选 schema 验证）。
...
* Gateway 网关/节点审批和允许列表：[执行审批](/07-tools/exec-approvals)。
...
* 参见 [Web 工具](/07-tools/web) 了解设置。
* 参见 [Firecrawl](/07-tools/firecrawl) 了解可选的反机器人回退。
