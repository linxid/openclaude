...
Hooks 也可以捆绑在插件中；参见 [插件](/openclaw/tools/plugin#plugin-hooks)。
...
* **`tool_result_persist`**：在工具结果写入会话记录之前转换它们。必须是同步的；返回更新后的工具结果负载或 `undefined` 保持原样。参见 [智能体循环](/openclaw/concepts/agent-loop)。
