...
这些文件位于工作空间下（`agents.defaults.workspace`，默认 `~/.openclaw/workspace`）。完整布局参见[智能体工作空间](/openclaw/concepts/agent-workspace)。
...
完整的压缩生命周期参见[会话管理 + 压缩](/openclaw/reference/session-management-compaction)。
