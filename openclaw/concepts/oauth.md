...
以上所有路径也遵循 `$OPENCLAW_STATE_DIR`（状态目录覆盖）。完整参考：[/gateway/configuration](/openclaw/gateway/configuration#auth-storage-oauth--api-keys)
...
* [/concepts/model-failover](/openclaw/concepts/model-failover)（轮换 + 冷却规则）
* [/tools/slash-commands](/openclaw/tools/slash-commands)（命令界面）
