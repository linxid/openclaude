...
* 消息渠道可以选择队列模式（collect/steer/followup）来馈送此通道系统。参见[命令队列](/openclaw/concepts/queue)。
...
* 强制执行模型特定的限制和压缩保留令牌。
* 参见[系统提示](/openclaw/concepts/system-prompt)了解模型看到的内容。
...
参见[插件](/openclaw/tools/plugin#plugin-hooks)了解钩子 API 和注册详情。
...
* 参见[流式传输](/openclaw/concepts/streaming)了解分块和块回复行为。
...
* 参见[压缩](/openclaw/concepts/compaction)了解压缩管道。
