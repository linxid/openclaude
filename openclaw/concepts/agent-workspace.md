...
[`agents.defaults.sandbox`](/openclaw/gateway/sandboxing)（和/或每智能体沙箱配置）。
...
参见 [记忆](/openclaw/concepts/memory) 了解工作流程和自动记忆刷新。
...
* 多智能体路由可以为每个智能体使用不同的工作区。参见
  [渠道路由](/openclaw/channels/channel-routing) 了解路由配置。
