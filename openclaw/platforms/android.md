...
* 安装：[入门指南](/openclaw/start/getting-started) + [配对](/openclaw/gateway/pairing)。
* Gateway 网关：[操作手册](/gateway) + [配置](/openclaw/gateway/configuration)。
  * 协议：[Gateway 网关协议](/openclaw/gateway/protocol)（节点 + 控制平面）。
...
更多调试说明：[Bonjour](/openclaw/gateway/bonjour)。
...
详情和示例 CoreDNS 配置：[Bonjour](/openclaw/gateway/bonjour)。
...
配对详情：[Gateway 网关配对](/openclaw/gateway/pairing)。
...
参见 [Camera 节点](/openclaw/nodes/camera) 了解参数和 CLI 助手。
