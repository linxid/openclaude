...
* **沙箱注意事项**：智能体沙箱隔离也使用 Docker，但它**不需要**完整的 Gateway 网关在 Docker 中运行。参阅[沙箱隔离](/openclaw/gateway/sandboxing)。
...
沙箱隔离详情：[沙箱隔离](/openclaw/gateway/sandboxing)
...
在 VPS 上运行？参阅 [Hetzner（Docker VPS）](/openclaw/install/hetzner)。
...
更多详情：[仪表板](/openclaw/web/dashboard)，[设备](/openclaw/cli/devices)。
...
文档：[WhatsApp](/openclaw/channels/whatsapp)，[Telegram](/openclaw/channels/telegram)，[Discord](/openclaw/channels/discord)
...
深入了解：[沙箱隔离](/openclaw/gateway/sandboxing)
...
参阅[多智能体沙箱与工具](/openclaw/tools/multi-agent-sandbox-tools)了解示例、优先级和故障排除。
