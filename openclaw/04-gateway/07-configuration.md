...
> **初次接触配置？** 请查阅[配置示例](/04-gateway/configuration-examples)指南，获取带有详细说明的完整示例！
...
参见 [/environment](/18-help/environment) 了解优先级和来源详情。
...
另请参阅：[/03-concepts/oauth](/03-concepts/oauth)
...
参见[多智能体沙箱与工具](/07-tools/multi-agent-sandbox-tools)了解优先级和更多示例。
...
* `/reasoning stream` 将推理过程流式传输到草稿中，然后发送最终答案。
  重试策略默认值和行为记录在[重试策略](/03-concepts/retry)中。
...
控制入站/出站前缀和可选的确认反应。
参见[消息](/03-concepts/messages)了解排队、会话和流式上下文。
...
参见 [/03-concepts/session-pruning](/03-concepts/session-pruning) 了解行为细节。

#### `agents.defaults.compaction`（预留空间 + 记忆刷新）

`agents.defaults.compaction.mode` 选择压缩摘要策略。默认为 `default`；设为 `safeguard` 可为超长历史启用分块摘要。参见 [/03-concepts/compaction](/03-concepts/compaction)。
...
* `agents.defaults.humanDelay`：第一条之后**块回复**之间的随机延迟。
  ...
  ```
  参见 [/03-concepts/streaming](/03-concepts/streaming) 了解行为 + 分块细节。
...
* `session.typingIntervalSeconds`：每会话的刷新间隔覆盖。
  参见 [/03-concepts/typing-indicators](/03-concepts/typing-indicators) 了解行为细节。
...
* 工具策略：仅允许 `exec`、`process`、`read`、`write`、`edit`、`apply_patch`、`sessions_list`、`sessions_history`、`sessions_send`、`sessions_spawn`、`session_status`（deny 优先）
  ...
  * 沙箱策略中支持工具组简写：`group:runtime`、`group:fs`、`group:sessions`、`group:memory`（参见[沙箱 vs 工具策略 vs 提升](/04-gateway/sandbox-vs-tool-policy-vs-elevated#tool-groups-shorthands)）
...
按提供商的概述 + 示例：[/03-concepts/model-providers](/03-concepts/model-providers)。
...
参见 [/04-gateway/local-models](/04-gateway/local-models) 了解当前本地指南。简而言之：在高性能硬件上通过 LM Studio Responses API 运行 MiniMax M2.1；保留托管模型合并作为回退。
...
参见 [/plugin](/07-tools/plugin) 了解详情。
...
参见[多 Gateway 网关](/04-gateway/multiple-gateways) 了解浏览器/CDP 端口隔离细节。
...
* `POST /13-hooks/wake` → `{ text, mode?: "now"|"next-heartbeat" }`
* `POST /13-hooks/agent` → `{ message, name?, sessionKey?, wakeMode?, deliver?, channel?, to?, model?, thinking?, timeoutSeconds? }`
* `POST /13-hooks/<name>` → 通过 `hooks.mappings` 解析

`/13-hooks/agent` 始终将摘要发布到主会话（并可通过 `wakeMode: "now"` 可选地触发即时心跳）。

映射说明：

* `match.path` 匹配 `/hooks` 之后的子路径（例如 `/13-hooks/gmail` → `gmail`）。
* `match.source` 匹配负载字段（例如 `{ source: "gmail" }`），以便使用通用的 `/13-hooks/ingest` 路径。
...
```json5  theme={null}
{
  hooks: {
    gmail: {
      ...
      hookUrl: "http://127.0.0.1:18789/13-hooks/gmail",
      includeBody: true,
      maxBytes: 20000,
...
Cron 是 Gateway 网关自有的唤醒和定时任务调度器。参见 [Cron 任务](/08-automation/cron-jobs) 了解功能概述和 CLI 示例。
