...
另请参阅：[形式化验证（安全模型）](/15-security/formal-verification/)
...
斜杠命令和指令仅对**授权发送者**有效。授权来源于渠道白名单/配对加上 `commands.useAccessGroups`（参见[配置](/04-gateway/configuration)和[斜杠命令](/07-tools/slash-commands)）。如果渠道白名单为空或包含 `"*"`，则该渠道的命令实际上是开放的。
...
详情 + 磁盘上的文件：[配对](/06-channels/pairing)
...
这可以防止跨用户上下文泄露，同时保持群聊隔离。如果你在同一渠道上运行多个账户，请改用 `per-account-channel-peer`。如果同一个人通过多个渠道联系你，请使用 `session.identityLinks` 将这些私信会话合并为一个规范身份。参见[会话管理](/03-concepts/session)和[配置](/04-gateway/configuration)。
...
详情：[配置](/04-gateway/configuration)和[群组](/06-channels/groups)
...
参见 [Tailscale](/04-gateway/tailscale) 和 [Web 概述](/web)。

### 0.6.1）通过节点主机进行浏览器控制（推荐）

如果你的 Gateway 网关是远程的但浏览器在另一台机器上运行，请在浏览器机器上运行一个**节点主机**，让 Gateway 网关代理浏览器操作（参见[浏览器工具](/07-tools/browser)）。将节点配对视为管理员访问。
...
专用文档：[沙箱隔离](/04-gateway/sandboxing)

两种互补的方法：

* **在 Docker 中运行完整的 Gateway 网关**（容器边界）：[Docker](/02-install/docker)
* **工具沙箱**（`agents.defaults.sandbox`，宿主机 Gateway 网关 + Docker 隔离的工具）：[沙箱隔离](/04-gateway/sandboxing)
...
重要：`tools.elevated` 是在宿主机上运行 exec 的全局基线逃逸舱口。保持 `tools.elevated.allowFrom` 严格，不要为陌生人启用它。你可以通过 `agents.list[].tools.elevated` 进一步限制单智能体的提权。参见[提权模式](/07-tools/elevated)。
...
通过多智能体路由，每个智能体可以有自己的沙箱 + 工具策略：使用这个为每个智能体提供**完全访问**、**只读**或**无访问**权限。参见[多智能体沙箱和工具](/07-tools/multi-agent-sandbox-tools)了解详情和优先级规则。
