...
* **人类签到**："Checkup sometimes on your human during day time"促使偶尔发送轻量级的"有什么需要帮助的吗？"消息，但通过使用你配置的本地时区避免夜间打扰（参见 [/03-concepts/timezone](/03-concepts/timezone)）。
...
* `session`：心跳运行的可选会话键。
  * `main`（默认）：智能体主会话。
  * 显式会话键（从 `openclaw sessions --json` 或 [sessions CLI](/05-cli/sessions) 复制）。
  * 会话键格式：参见[会话](/03-concepts/session)和[群组](/06-channels/groups)。
* `target`：
