...
如果你只想快速分类问题，请先查看常见问题的[最初的六十秒](/18-help/faq#first-60-seconds-if-somethings-broken)。本页深入介绍运行时故障和诊断。

特定提供商的快捷方式：[/06-channels/troubleshooting](/06-channels/troubleshooting)
...
另请参阅：[健康检查](/04-gateway/health) 和 [日志](/logging)。
...
更多详情：[Anthropic](/11-providers/anthropic) 和 [OAuth](/03-concepts/oauth)。
...
* 优先通过 [Tailscale Serve](/04-gateway/tailscale) 使用 HTTPS。
...
* 如果必须使用 HTTP，启用 `gateway.controlUi.allowInsecureAuth: true` 并
  ...
  [Control UI](/14-web/control-ui#insecure-http)。

### CI Secrets Scan Failed

这意味着 `detect-secrets` 发现了尚未在基线中的新候选项。
按照 [密钥扫描](/04-gateway/security#secret-scanning-detect-secrets) 操作。
...
`tools.exec.pathPrepend`）。参见 [/07-tools/exec](/07-tools/exec)。
...
[智能体工作区](/03-concepts/agent-workspace#extra-workspace-folders)。
...
另请参阅：[模型 CLI](/05-cli/models) 和 [模型提供商](/03-concepts/model-providers)。
...
参见 [WhatsApp 设置](/06-channels/whatsapp)。
...
文档：[Discord](/06-channels/discord)、[渠道故障排除](/06-channels/troubleshooting)。
...
参见 [工具](/tools) 和 [TypeBox 模式](/03-concepts/typebox)。
...
**完整指南：** 参见 [browser-linux-troubleshooting](/07-tools/browser-linux-troubleshooting)
