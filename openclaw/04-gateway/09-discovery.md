...
* **旧版 TCP 桥接（已弃用/移除）**：旧的节点传输（参见 [桥接协议](/04-gateway/bridge-protocol)）；不再用于发现广播。

协议详情：

* [Gateway 网关协议](/04-gateway/protocol)
* [桥接协议（旧版）](/04-gateway/bridge-protocol)
...
故障排除和信标详情：[Bonjour](/04-gateway/bonjour)。
...
* 配对请求在 Gateway 网关中创建/批准/拒绝（参见 [Gateway 网关配对](/04-gateway/pairing)）。
* Gateway 网关强制执行：
  * 认证（令牌 / 密钥对）
