...
* 任何明确允许在主机上运行的工具（例如 `tools.elevated`）。
  ...
  * 如果沙箱隔离关闭，`tools.elevated` 不会改变执行（已经在主机上）。参见[提权模式](/07-tools/elevated)。
...
* 参见[沙箱 vs 工具策略 vs 提权](/04-gateway/sandbox-vs-tool-policy-vs-elevated)了解绑定如何与工具策略和提权 exec 交互。
...
`/exec` 指令仅适用于授权发送者并按会话持久化；要硬禁用 `exec`，使用工具策略拒绝（参见[沙箱 vs 工具策略 vs 提权](/04-gateway/sandbox-vs-tool-policy-vs-elevated)）。

调试：

* 使用 `openclaw sandbox explain` 检查生效的沙箱模式、工具策略和修复配置键。
* 参见[沙箱 vs 工具策略 vs 提权](/04-gateway/sandbox-vs-tool-policy-vs-elevated)了解"为什么被阻止？"的心智模型。
  保持锁定。
...
参见[多智能体沙箱与工具](/07-tools/multi-agent-sandbox-tools)了解优先级。
...
* [沙箱配置](/04-gateway/configuration#agentsdefaults-sandbox)
* [多智能体沙箱与工具](/07-tools/multi-agent-sandbox-tools)
