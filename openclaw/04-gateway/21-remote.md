...
* **示例：** [exe.dev](/02-install/exe-dev)（简易 VM）或 [Hetzner](/02-install/hetzner)（生产 VPS）。
...
操作手册：[macOS 远程访问](/10-platforms/mac/remote)。
...
指南：[Tailscale](/04-gateway/tailscale) 和 [Web 概览](/web)。
...
* **节点不运行 Gateway 网关服务。** 除非你有意运行隔离的配置文件，否则每台主机只应运行一个 Gateway 网关（参见[多 Gateway 网关](/04-gateway/multiple-gateways)）。
