...
* [/gateway/bonjour](/openclaw/gateway/bonjour)
* [/gateway/discovery](/openclaw/gateway/discovery)
* [/gateway/configuration](/openclaw/gateway/configuration)
...
* 组播 DNS-SD：`local.`
* 单播 DNS-SD（广域 Bonjour）：选择一个域（示例：`openclaw.internal.`）并设置分割 DNS + DNS 服务器；参见 [/gateway/bonjour](/openclaw/gateway/bonjour)
