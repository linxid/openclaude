...
最快开始聊天的方式：打开控制界面（无需设置渠道）。运行 `openclaw dashboard` 并在浏览器中聊天。文档：[控制面板](/14-web/dashboard)。
...
推荐：设置 Brave Search API 密钥，以便智能体可以使用 `web_search`（`web_fetch` 无需密钥即可使用）。最简单的方式：`openclaw configure --section web`，它会存储 `tools.web.search.apiKey`。文档：[Web 工具](/07-tools/web)。
...
2. **模型/认证**
   ...
   * 更多详情：[Vercel AI Gateway](/11-providers/vercel-ai-gateway)
   * **MiniMax M2.1**：自动写入配置。
   * 更多详情：[MiniMax](/11-providers/minimax)
   * **Synthetic（Anthropic 兼容）**：提示输入 `SYNTHETIC_API_KEY`。
   * 更多详情：[Synthetic](/11-providers/synthetic)
   * **Moonshot（Kimi K2）**：自动写入配置。
   * **Kimi Coding**：自动写入配置。
   * 更多详情：[Moonshot AI（Kimi + Kimi Coding）](/11-providers/moonshot)
   * **跳过**：尚未配置认证。
...
3. **工作区**
   ...
   * 为智能体引导仪式播种所需的工作区文件。
   * 完整的工作区布局 + 备份指南：[智能体工作区](/03-concepts/agent-workspace)
...
5. **渠道**
   * [WhatsApp](/06-channels/whatsapp)：可选的二维码登录。
   * [Telegram](/06-channels/telegram)：机器人令牌。
   * [Discord](/06-channels/discord)：机器人令牌。
   * [Google Chat](/06-channels/googlechat)：服务账户 JSON + webhook 受众。
   * [Mattermost](/06-channels/mattermost)（插件）：机器人令牌 + 基础 URL。
   * [Signal](/06-channels/signal)：可选的 `signal-cli` 安装 + 账户配置。
   * [iMessage](/06-channels/imessage)：本地 `imsg` CLI 路径 + 数据库访问。
...
* 下载适当的发布资源。
* 存储在 `~/.openclaw/07-tools/signal-cli/<version>/` 下。
...
* macOS 应用新手引导：[新手引导](/01-start/onboarding)
* 配置参考：[Gateway 网关配置](/04-gateway/configuration)
* 提供商：[WhatsApp](/06-channels/whatsapp)、[Telegram](/06-channels/telegram)、[Discord](/06-channels/discord)、[Google Chat](/06-channels/googlechat)、[Signal](/06-channels/signal)、[iMessage](/06-channels/imessage)
* Skills：[Skills](/07-tools/skills)、[Skills 配置](/07-tools/skills-config)
