...
* 同一端口也提供 HTTP 服务（控制界面、hooks、A2UI）。单端口多路复用。
  * OpenAI Chat Completions（HTTP）：[`/v1/chat/completions`](/openclaw/gateway/openai-http-api)。
  * OpenResponses（HTTP）：[`/v1/responses`](/openclaw/gateway/openresponses-http-api)。
  * Tools Invoke（HTTP）：[`/tools/invoke`](/openclaw/gateway/tools-invoke-http-api)。
...
如果你隔离状态 + 配置并使用唯一端口，则支持。完整指南：[多个 Gateway 网关](/openclaw/gateway/multiple-gateways)。
...
救援机器人模式：保持第二个 Gateway 网关隔离，使用自己的配置文件、状态目录、工作区和基础端口间隔。完整指南：[救援机器人指南](/openclaw/gateway/multiple-gateways#rescue-bot-guide)。
...
* 完整文档：[Gateway 网关协议](/openclaw/gateway/protocol) 和 [Bridge 协议（旧版）](/openclaw/gateway/bridge-protocol)。
...
另见：[Presence](/openclaw/concepts/presence) 了解 presence 如何产生/去重以及为什么稳定的 `client.instanceId` 很重要。
...
* 如果检测到其他类似 Gateway 网关的服务，CLI 会发出警告，除非它们是 OpenClaw 配置文件服务。
  我们仍然建议大多数设置**每台机器一个 Gateway 网关**；使用隔离的配置文件/端口进行冗余或救援机器人。参见[多个 Gateway 网关](/openclaw/gateway/multiple-gateways)。
