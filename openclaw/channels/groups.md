...
> 如果你需要真正独立的工作区/角色（"个人"和"公开"绝不能混合），请使用第二个智能体 + 绑定。参见[多智能体路由](/openclaw/concepts/multi-agent)。
...
* 配置键和默认值：[Gateway 网关配置](/openclaw/gateway/configuration#agentsdefaultssandbox)
* 调试为什么工具被阻止：[沙箱 vs 工具策略 vs 提权](/openclaw/gateway/sandbox-vs-tool-policy-vs-elevated)
* 绑定挂载详情：[沙箱隔离](/openclaw/gateway/sandboxing#custom-bind-mounts)
...
参见[群消息](/openclaw/channels/group-messages)了解 WhatsApp 专有行为（历史注入、提及处理详情）。
