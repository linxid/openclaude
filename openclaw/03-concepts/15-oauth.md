...
以上所有路径也遵循 `$OPENCLAW_STATE_DIR`（状态目录覆盖）。完整参考：[/04-gateway/configuration](/04-gateway/configuration#auth-storage-oauth--api-keys)
...
* [/03-concepts/model-failover](/03-concepts/model-failover)（轮换 + 冷却规则）
* [/07-tools/slash-commands](/07-tools/slash-commands)（命令界面）
