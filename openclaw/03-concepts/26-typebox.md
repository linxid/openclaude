...
如果你想了解更高层次的协议上下文，请从 [Gateway 网关架构](/03-concepts/architecture)开始。
...
权威列表在 `src/04-gateway/server.ts`（`METHODS`、`EVENTS`）中。

## 模式所在位置

* 源码：`src/04-gateway/protocol/schema.ts`
* 运行时验证器（AJV）：`src/04-gateway/protocol/index.ts`
* 服务器握手 + 方法分发：`src/04-gateway/server.ts`
* 节点客户端：`src/04-gateway/client.ts`
...
添加到 `src/04-gateway/protocol/schema.ts`：
...
在 `src/04-gateway/protocol/index.ts` 中，导出一个 AJV 验证器：
...
在 `src/04-gateway/server-methods/system.ts` 中添加处理程序：
...
在 `src/04-gateway/server-methods.ts` 中注册（已合并 `systemHandlers`），然后将 `"system.echo"` 添加到 `src/04-gateway/server.ts` 中的 `METHODS`。
...
在 `src/04-gateway/server.*.test.ts` 中添加服务器测试，并在文档中记录该方法。
...
* `PROTOCOL_VERSION` 在 `src/04-gateway/protocol/schema.ts` 中。
