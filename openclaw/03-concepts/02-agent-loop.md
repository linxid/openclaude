...
* 消息渠道可以选择队列模式（collect/steer/followup）来馈送此通道系统。参见[命令队列](/03-concepts/queue)。
...
* 强制执行模型特定的限制和压缩保留令牌。
* 参见[系统提示](/03-concepts/system-prompt)了解模型看到的内容。
...
参见[钩子](/08-automation/hooks)了解设置和示例。
...
参见[插件](/07-tools/plugin#plugin-hooks)了解钩子 API 和注册详情。
...
* 参见[流式传输](/03-concepts/streaming)了解分块和块回复行为。
...
* 参见[压缩](/03-concepts/compaction)了解压缩管道。
