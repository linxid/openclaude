...
有关 `agents.defaults.compaction` 设置，请参阅[压缩配置与模式](/03-concepts/compaction)。
...
在压缩之前，OpenClaw 可以运行一次**静默记忆刷写**轮次，将持久化笔记写入磁盘。详情及配置请参阅[记忆](/03-concepts/memory)。
...
有关修剪的详情，请参阅 [/03-concepts/session-pruning](/03-concepts/session-pruning)。
