...
另请参阅：[斜杠命令](/07-tools/slash-commands)、[Token 使用与成本](/17-reference/token-use)、[压缩](/03-concepts/compaction)。
...
完整分解：[系统提示词](/03-concepts/system-prompt)。
...
详情：[斜杠命令](/07-tools/slash-commands)。
...
文档：[会话](/03-concepts/session)、[压缩](/03-concepts/compaction)、[会话修剪](/03-concepts/session-pruning)。
