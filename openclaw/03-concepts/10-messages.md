...
完整 schema 参见[配置](/04-gateway/configuration)。
...
详情：[流式传输 + 分块](/03-concepts/streaming)。
...
详情：[思考 + 推理指令](/07-tools/thinking)和 [Token 使用](/17-reference/token-use)。
...
详情：[配置](/04-gateway/configuration#messages)和渠道文档。
