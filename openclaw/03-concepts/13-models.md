...
参见 [/03-concepts/model-failover](/03-concepts/model-failover) 了解认证配置文件轮换、冷却时间及其与回退的交互。
快速提供商概述 + 示例：[/03-concepts/model-providers](/03-concepts/model-providers)。
...
* 每个智能体的默认值可以通过 `agents.list[].model` 加绑定覆盖 `agents.defaults.model`（参见 [/03-concepts/multi-agent](/03-concepts/multi-agent)）。
...
提供商配置示例（包括 OpenCode Zen）在 [/04-gateway/configuration](/04-gateway/configuration#opencode-zen-multi-model-proxy)。
...
完整命令行为/配置：[斜杠命令](/07-tools/slash-commands)。
...
* 需要来自认证配置文件或 `OPENROUTER_API_KEY` 的 OpenRouter API 密钥（参见 [/environment](/18-help/environment)）
